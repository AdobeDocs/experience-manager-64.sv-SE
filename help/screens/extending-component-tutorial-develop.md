...
>[!NOTE]
>
>Innan du startar den här självstudiekursen rekommenderar vi att du slutför självstudiekursen: [Utveckla en anpassad komponent för AEM-skärmar](/help/screens/developing-custom-component-tutorial-develop.md).
...
1. Hämta och installera följande paket med [CRX-](http://localhost:4502/crx/packmgr/index.jsp)pakethanteraren:
...
1. Följande två paket är installerade i [CRX Package Manager](http://localhost:4502/crx/packmgr/index.jsp) :
...
1. I [CRXDE-Lite](http://localhost:4502/crx/de/index.jsp) (eller valfri IDE) nedan `/apps/weretail-run/components/content`skapar du ett nytt `cq:Component` med namnet `poster`.
...
1. Kopiera `image` dialogrutan WCM Foundation som ska användas för `poster` komponenten.
   ...
   Skärmkomponenten är `image` supertypad till WCM Foundation- `image` komponenten. Därför ärver komponenten funktioner från båda `poster` programmen. Dialogrutan för förhandsgranskningskomponenten består av en kombination av dialogrutorna Skärmar och Foundation. Funktionerna i [Sling Resource Merger](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/sling-resource-merger.html) används för att dölja irrelevanta dialogrutefält och flikar som ärvs från de överordnade komponenterna.
