...
Här kan du navigera bland bilderna (genom att klicka på siffrorna). Dessutom roteras bildrutorna automatiskt baserat på ett tidsintervall som du kan anpassa. Bilder som du lägger till i karusellbanderollen har stöd för både aktiveringspunkter och bildscheman, där användarna kan trycka eller gå till en hyperlänk eller öppna ett Quickview-fönster.
...
1. [Identifiera hotspot- och bildschemavariabler](#identifying-hotspot-and-image-map-variables) (endast för kunder som använder AEM Assets + Dynamic Media)
   ...
   >[!NOTE]
   >
   >Om du är kund hos AEM Sites eller e-handel kan du använda den inbyggda funktionen för att navigera till produktsidor och söka efter befintliga skal i produktkatalogen. Du behöver inte ange variabler för hotspot eller bildschema manuellt. Se information om [konfigurera e-handel](/help/sites-administering/generic.md).
   >
...
1. Valfritt: [Skapa en visningsförinställning för en karuselluppsättning](managing-viewer-presets.md) om det behövs.
...
1. [Publicera Carousel Banners.](#publishing-carousel-banners)
   ...
   Du publicerar en Carousel-uppsättning på samma sätt som andra resurser. I Assets navigerar du till Carousel Set och markerar den och trycker eller trycker på **[!UICONTROL Publish]**. När du publicerar en Carousel Set aktiveras URL:en och strängen Embed.
...
1. Gör något av följande:
   ...
   * [Lägg till en karusellbanderoll på din webbsida](#adding-a-carousel-banner-to-your-website-page) Du kan lägga till karusellbanderollens URL eller inbäddningskod som du har kopierat till webbsidan.
      ...
      * [Integrera karusellbanderollen med en befintlig snabbvy](#integrating-the-carousel-banner-with-an-existing-quickview). Om du använder ett webbinnehållshanteringssystem från tredje part måste du integrera den nya Carousel-banderollen med den befintliga Quickview-implementeringen på din webbplats.
...
Om du behöver redigera Carousel-uppsättningar, se [redigera Carousel-uppsättningar](#editing-carousel-sets). Dessutom kan du visa och redigera [Carousel Set-egenskaper](/help/assets/managing-assets-touch-ui.md#editing-properties).
...
* Användaren aktiverar ett element i användargränssnittet på webbplatsen. Klicka till exempel på en **[!UICONTROL Quick View]** -knappen.
...
* Om du vill visa alla utgående HTTP-begäranden i Google Chrome trycker du på F12 (Windows) eller Command-Option-I (Mac) för att öppna panelen Utvecklarverktyg och trycker sedan på **[!UICONTROL Network]** -fliken.
 ...
 <tbody> 
  <tr> 
   ...
   <td><p>De inspelade URL:erna för snabbvyn är bland annat följande:</p> 
    ...
    </ul> <p>Den enda variabeldelen i URL:en är värdet på <code>productId=</code> frågesträngsparameter, och det är tydligt ett SKU-värde. Därför behöver våra aktiveringspunkter eller bildscheman bara SKU-fält med värden som <code>866558,</code> <code>1196184,</code> <code>1081492,</code> <code>1898294.</code></p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td><p>De inspelade URL:erna för snabbvyn är bland annat följande:</p> 
    ...
    </ul> <p>I det här fallet finns det två olika delar i URL:en. SKU:n lagras i <code>prodId</code> -parametern och kategori-ID:t lagras i <code>category=</code>parameter.</p> <p>Definitionerna av hotspot/bildschema är par. Det vill säga ett SKU-värde och en ytterligare variabel som kallas <code>categoryId</code>. De resulterande paren är följande:</p> 
    <ul> 
...
Information om hur du överför bildbanderoller finns i [Överför resurser](managing-assets-touch-ui.md).

## Skapa Carousel-uppsättningar {#creating-carousel-sets}

>[!NOTE]
>
>Icke-administrativa användare måste läggas till i **[!UICONTROL dam-users]** för att kunna skapa eller redigera karusellbanners. Om du har problem med att skapa eller redigera kontaktar du systemadministratören som kan lägga till dig i **dam-users** grupp.
...
1. På **[!UICONTROL Carousel Banner Editor]** sida, tryck **[!UICONTROL Tap to open Asset Selector]** för att välja bilden för den första bilden.

   På **[!UICONTROL Carousel Banner Editor]** gör du något av följande:

   * Knacka i det övre vänstra hörnet av sidan **[!UICONTROL Add Slide]** ikon.
   * I mitten av sidan trycker du på **[!UICONTROL Tap to open Asset Selector]**.

   Tryck här för att välja resurser som du vill inkludera i karuselluppsättningen. De markerade resurserna visas med en bock. När du är klar trycker du på **[!UICONTROL Select]**.
   ...
   Se [Arbeta med väljare](working-with-selectors.md) för mer information.
...
1. (Valfritt) Gör något av följande:
   ...
   * Om du vill ta bort en bild markerar du bilden och trycker sedan **[!UICONTROL Delete Slide]** i verktygsfältet.
   * Om du vill använda en förinställning trycker du på den förinställda listrutan längst upp till höger på sidan och väljer sedan en förinställning som ska användas på uppsättningen samtidigt.
   ...
   Om du vill ta bort en bildruta trycker du på bildrutan och trycker på **[!UICONTROL Delete Slide]** i verktygsfältet. Om du vill flytta en bildruta trycker du på ikonen för att ändra ordning och håller ned och flyttar till önskad plats.

1. När du har lagt till bilderna i bildrutor kan du lägga till en aktiveringspunkt, ett bildschema eller båda delarna. Se [lägga till hotspot-områden eller bildscheman](#adding-hotspots-or-image-maps-to-an-image-banner).
1. Du kan ändra den visuella designen och beteendet för karuselluppsättningar genom att trycka eller klicka på flikarna Beteende och Utseende och göra justeringar av hur karusellbanderollen ser ut eller hur specifika komponenter fungerar. Se [hantera visningsförinställningar](viewer-presets.md) om du vill ha mer information om hur du använder visningsprogrammet.
...
Kom ihåg att spara ditt arbete när du lägger till hotspot-områden eller bildscheman i en bild. **[!UICONTROL Undo]** och **[!UICONTROL Redo]** alternativen, som finns uppe till höger på sidan, stöds under den aktuella skaps-/redigeringssessionen.

När du är klar med att skapa en karusellbanderoll kan du använda **[!UICONTROL Preview]** för att se hur karusellbanderollen kommer att se ut för kunderna.
...
1. Välj Carousel-uppsättning och tryck **[!UICONTROL Edit]**.
...
1. Skriv namnet på aktiveringspunkten eller bildschemat i textfältet Namn. Det här namnet visas även i **[!UICONTROL Maps & Hotspot]** nedrullningsbar lista. Genom att ange ett namn blir det enkelt att identifiera hotspot-området eller bildschemat om du bestämmer dig för att göra ändringar i det i framtiden.
1. Gör något av följande i dialogrutan **[!UICONTROL Actions]** tab:

   * Tryck på **[!UICONTROL Quickview]**.

      * Om du är kund hos AEM Sites och e-handel trycker du på **[!UICONTROL Product Picker]** ikon (förstoringsglas) för att öppna **[!UICONTROL Select Product]** sida. Tryck på den produkt du vill använda och tryck sedan på bockmarkeringen i det övre högra hörnet av sidan för att gå tillbaka till **[!UICONTROL Carousel Banner Editor]**.
      * Om du inte är AEM Sites- eller e-handelskund

         * Se [Identifiera hotspot-variabler](#identifying-hotspot-and-image-map-variables) som du kanske vill definiera dessa variabler.
         * Ange sedan SKU-värdet manuellt. I **[!UICONTROL SKU Value]** ska du skriva produktens SKU (Stock Keeping Unit), som är en unik identifierare för varje separat produkt eller tjänst som du erbjuder. Det angivna SKU-värdet fyller automatiskt i variabeldelen av QuickView-mallen så att systemet vet att den aktiveringspunkt som användaren går till associeras med en viss SKU:s snabbvy.
         * (Valfritt) Om det finns andra variabler i snabbvyn som du behöver använda för att identifiera en produkt ytterligare trycker du på **[!UICONTROL Add Generic Variable]**. Ange ytterligare en variabel i textfältet. Till exempel: `category=Mens` är en tillagd variabel.
         * Se [Arbeta med väljare](working-with-selectors.md) för mer information.
   * Tryck på **[!UICONTROL Hyperlink]**.

      * Om du är AEM Sites-kund trycker du på **[!UICONTROL Site Selector]** -ikon (mapp) för att navigera till en URL.
      ...
      * Om du är en fristående kund kan du **[!UICONTROL HREF]** anger du den fullständiga URL-sökvägen till en länkad webbsida.
   ...
   * Tryck på **[!UICONTROL Experience Fragment]**.
      ...
      * Om du är AEM Sites-kund trycker du på **[!UICONTROL Search]** ikon (förstoringsglas) för att öppna sidan Experience Fragment. Tryck på det Experience Fragment du vill använda och tryck sedan på **[!UICONTROL Select]** i det övre högra hörnet av sidan för att gå tillbaka till sidan för hantering av hotspot.
...
1. Gör något av följande:
   ...
   * Håll pekaren över en Carousel Set-resurs och tryck sedan **[!UICONTROL Edit]** (pennikon).
   * Hovra över en Carousel Set-resurs, tryck **[!UICONTROL Select]** (bockmarkeringsikon) och sedan trycka **[!UICONTROL Edit]** i verktygsfältet.
   * Tryck på en Carousel Set-resurs och tryck sedan på i det övre vänstra hörnet av sidan **[!UICONTROL Edit]** (pennikon).

1. Om du vill redigera Carousel-uppsättningen gör du något av följande:

   * Om du vill lägga till en bildruta trycker du på **[!UICONTROL Add Slide]** navigera sedan till den resurs som du vill lägga till i bildrutan och tryck på bockmarkeringen.
   ...
   * Om du vill lägga till en hotspot eller ett bildschema trycker du på ikonerna för hotspot eller bildschema och ser [lägga till hotspot-områden och bildscheman](#adding-hotspots-or-image-maps-to-an-image-banner).
   * Om du vill redigera karuselluppsättningens utseende eller beteende trycker du på **[!UICONTROL Appearance]** tabb eller **[!UICONTROL Behavior]** och sedan ange önskade alternativ.
...
Du kan använda **[!UICONTROL Preview]** för att se hur karusellbanderollen kommer att se ut för kunderna och för att testa hotspot-områden och bildscheman för att säkerställa att de beter sig som förväntat.
...
* Se [Länka URL:er till webbprogrammet](linking-urls-to-yourwebapplication.md). Observera att den URL-baserade länkningsmetoden inte är möjlig om det interaktiva innehållet har länkar till relativa URL-adresser, särskilt länkar till AEM Sites-sidor.
* Se [Lägga till Dynamic Media Assets på sidor.](adding-dynamic-media-assets-to-pages.md)
...
1. I **[!UICONTROL Assets]** navigera till en befintlig karusellbanderoll som du har skapat och tryck för att öppna den.
...
1. I **[!UICONTROL Assets]** navigera till en befintlig karusellbanderoll som du har skapat och tryck för att öppna den.
1. Nära det övre vänstra hörnet av **[!UICONTROL Preview]** tryck på **[!UICONTROL Content]** ikon.
1. I **[!UICONTROL Viewers]** i panelen till vänster på sidan trycker du på namnet på den visningsförinställning för Carousel banner som du vill använda.
1. Tryck på hotspot-områden eller bildscheman på bilden för att testa deras associerade åtgärder.
...
Se [Publicera Dynamic Media Assets](publishing-dynamicmedia-assets.md) om du vill ha information om hur du publicerar Carousel-banners.
...
Om du är kund hos AEM Sites kan du lägga till karusellbanderollen direkt på din sida genom att dra Interactive Media-komponenten till din sida. Se [Lägga till Dynamic Media-resurser på sidor.](adding-dynamic-media-assets-to-pages.md)
