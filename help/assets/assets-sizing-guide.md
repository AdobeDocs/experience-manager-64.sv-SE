...
description: 'Bästa metoder för att fastställa effektiva mått för att uppskatta den infrastruktur och de resurser som krävs för att distribuera AEM Assets. '
...
Slutligen lagras dubbletter av resurserna i versionshistoriken i AEM:s versionshanteringsfunktioner. Du kan konfigurera versionerna så att de rensas ofta. Många användare väljer dock att behålla versioner i systemet under lång tid, vilket kräver ytterligare lagringsutrymme.
...
AEM Assets har ett antal användningsfall som gör nätverksprestanda viktigare än i många av våra AEM-projekt. En kund kan ha en snabb server, men om nätverksanslutningen inte är tillräckligt stor för att stödja belastningen på de användare som överför och hämtar resurser från systemet verkar den ändå vara långsam. Det finns en bra metod för att fastställa krympningspunkten i en användares nätverksanslutning till AEM vid [AEM Asset för användarupplevelser, till exempel storlek, utvärdering av arbetsflöde och nätverkstopologi](assets-network-considerations.md).

## WebDAV {#webdav}

Om du lägger till AEM-datorprogrammet i mixen blir nätverksproblemen svårare på grund av ineffektivitet i WebDAV-protokollet.

För att illustrera dessa ineffektiva funktioner testade Adobe systemets prestanda med WebDAV i OS X. En 3,5 MB InDesign-fil har öppnats, redigerats och sparats. Följande iakttagelser gjordes:
...
Mer information finns i [Felsöka AEM-datorprogrammet](https://helpx.adobe.com/experience-manager/kb/troubleshooting-companion-app.html).
...
Som standard kan du med AEM överföra resurser med en filstorlek på upp till 2 GB. Information om hur du överför mycket stora resurser i AEM finns i [Konfiguration för att överföra mycket stora resurser](managing-video-assets.md#configuration-to-upload-video-assets-that-are-larger-than-gb).
