...
För en ZooKeeper Ensemble anger du kommaavgränsade `HOST:PORT`-värden, till exempel *host1:2181,host2:2181*
...
>[!NOTE]
>
>MongoDB-databasen, som har standardvärdet `communities`, ska inte anges till namnet på en databas som används för [nodarkiv eller datalager (binära)](../../help/sites-deploying/data-store-config.md). Se även [Lagringselement i AEM 6](../../help/sites-deploying/storage-elements-in-aem-6.md).
...
För att åtgärda felet måste du se till att du följer instruktionerna för [Installera standard-MLS](solr.md#installing-standard-mls)
