...
1. I **[!UICONTROL User Accounts]** anger du autentiseringsuppgifter för ett lokalt användarkonto och klickar på **[!UICONTROL Submit]**. Tillåt åtkomst till användaren om Microsoft Windows tillfrågas. När den konfigurerade användaren läggs till visas den under **[!UICONTROL Your user accounts]** i **[!UICONTROL User Accounts]** -fliken.
...
1. Kör batchfilen för verktyget PDF Generator för att konfigurera Acrobat för tjänsten PDF Generator:
   ...
   1. Öppna [AEM](http://localhost:4502/crx/packmgr/index.jsp) och ladda ned `adobe-aemfd-pdfg-common-pkg-[version].zip` från pakethanteraren.
   1. Zippa upp den nedladdade ZIP-filen. Öppna kommandotolken med administratörsbehörighet.
   1. Navigera till `[extracted-zip-file]\jcr_root\etc\fd\pdfg\tools\adobe-aemfd-pdfg-utilities-[version]-win.zip\scripts` katalog. Kör följande kommandofil:
