...
När ett formulär är förifyllt eller skickat slussas inskickade data via AEM för datamassning till mellanliggande format. Data sparas inte på en AEM-instans förutom när det adaptiva formuläret använder Adobe Sign, verifiering, formulärportalutkast eller -sändning, eller AEM-arbetsflöden

Du kan konfigurera en skicka-åtgärd i delen **[!UICONTROL Skicka]** i egenskaperna för den adaptiva formulärbehållaren i sidlisten.

![](assets/thank-you-setting.png) Configure Submit Action **** Figure: *Konfigurera Skicka-åtgärd*
...
Alternativet **[!UICONTROL Skicka till REST-slutpunkt]** skickar data som är ifyllda i formuläret till en konfigurerad bekräftelsesida som en del av HTTP GET-begäran. Du kan lägga till namnet på fälten som ska begäras. Begäran har följande format:

`{fieldName}={request parameter name}`

Som visas i bilden nedan `param1` och `param2` skickas som parametrar med värden som kopieras från fälten för **[!UICONTROL textruta]** och **[!UICONTROL sifferruta]** för nästa åtgärd.

Du kan också **[!UICONTROL aktivera POST-begäran]** och ange en URL för att skicka begäran. Om du vill skicka data till AEM-servern som är värd för formuläret använder du en relativ sökväg som motsvarar rotsökvägen för AEM-servern. Exempel: /content/forms/af/SampleForm.html. Om du vill skicka data till en annan server använder du den absoluta sökvägen.
...
Använd åtgärden **[!UICONTROL Skicka till REST-slutpunkt]** för att skicka skickade data till en rest-URL. URL:en kan vara en intern (servern som formuläret återges på) eller en extern server.
...
När formuläret har skickats skickar åtgärden **[!UICONTROL Skicka e-post]** ett e-postmeddelande till en eller flera mottagare. E-postmeddelandet som genereras kan innehålla formulärdata i ett fördefinierat format.
...
När du skickar ett e-postmeddelande med en PDF-fil som innehåller formulärdata skickas ett e-postmeddelande med åtgärden **[!UICONTROL Skicka PDF via e-post]** till en eller flera mottagare när formuläret har skickats.
...
Alternativet Skicka **[!UICONTROL till formulär-arbetsflödet]** skickar en XML-datafil och eventuella bifogade filer till en befintlig Adobe LiveCycle- eller AEM Forms on JEE-process.
...
Åtgärden **[!UICONTROL Skicka med formulärdatamodell]** skriver skickade adaptiva formulärdata för det angivna datamodellobjektet i en formulärdatamodell till datakällan. När du konfigurerar skicka-åtgärden kan du välja ett datamodellsobjekt vars skickade data du vill skriva tillbaka till dess datakälla.
...
Alternativet **[!UICONTROL Skicka åtgärd]** för formulärportalen gör att formulärdata blir tillgängliga via en AEM Forms-portal.
...
Åtgärden **[!UICONTROL Anropa ett AEM-arbetsflöde]** kopplar ett anpassat formulär till ett AEM-arbetsflöde. När ett formulär skickas startar det associerade arbetsflödet automatiskt på bearbetningsnoden. Dessutom placeras datafilen, bilagorna och, om tillämpligt, arkivdokumentet vid arbetsflödets nyttolastplats.

Innan du använder åtgärden **[!UICONTROL Anropa ett AEM-arbetsflöde]** ska du [konfigurera AEM DS-inställningarna](/help/forms/using/configuring-the-processing-server-url-.md). Mer information om hur du skapar ett AEM-arbetsflöde finns i [Formulärbaserade arbetsflöden i OSGi](/help/forms/using/aem-forms-workflow.md).
...
![](assets/revalidate-on-server.png) Aktivera validering **på serversidan** Bild: *Aktivera validering på serversidan*
...
När det gäller **komplexa valideringsregler** finns ibland det exakta valideringsskriptet i anpassade funktioner och författaren anropar dessa anpassade funktioner från fältvalideringsuttryck. För att göra det här anpassade funktionsbiblioteket tillgängligt när du utför validering på serversidan kan formulärförfattaren konfigurera namnet på AEM-klientbiblioteket på fliken **[!UICONTROL Grundläggande]** i egenskaper för adaptiv formulärbehållare enligt nedan.

![](assets/clientlib-cat.png) Supporting Custom functions in Validation Expressions **** Figure: *Stöd för anpassade funktioner i valideringsuttryck*
