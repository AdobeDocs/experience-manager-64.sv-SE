...
1. ![dropobjectToInputField-updatedata](assets/dropobjectstoinputfield-updatedata.png)
   ...
   Dra och släpp fältet **[!UICONTROL Shipping Address, State, and Zip Code]** från fliken Formulärobjekt till motsvarande tabellnamnsegenskap (till exempel kundinformation .shippingAddress) för fältet **[!UICONTROL Drop object or select here]** i rutan **[!UICONTROL INPUT]**. Alla fält med tabellnamn som prefix (t.ex. kundinformation i det här fallet) fungerar som indata för uppdateringstjänsten. Allt innehåll i dessa fält uppdateras i datakällan.
...
1. Dra och släpp fältet **[!UICONTROL Customer ID]** från fliken Formulärobjekt till id-fältet i rutan **[!UICONTROL INPUT]**. Fält utan ett prefix-tabellnamn (t.ex. kundinformation i det här fallet) fungerar som sökparametrar för uppdateringstjänsten. Fältet **[!UICONTROL id]** i det här användningsfallet identifierar en post unikt i tabellen för kundinformation.
...
1. Tryck på fältet **[!UICONTROL Customer ID]** och tryck på ikonen `Edit Rules`. Regelredigeraren öppnas.
1. Tryck på ikonen **[!UICONTROL + Create]** för att lägga till en regel. Den öppnar Visual Editor.
