...
Använd ***LC_GLOBAL_ALLOWED_REFERER_EXCEPTION*** lista för tillåtna referensundantag på global nivå, dvs. för att definiera undantag som gäller för alla program. Den här listan innehåller bara URI:er med antingen en absolut sökväg (till exempel:  `/index.html`) eller en relativ sökväg (till exempel  `/sample/`). Du kan också lägga till ett reguljärt uttryck i slutet av en relativ URI, t.ex.  `/sample/(.)*`.
