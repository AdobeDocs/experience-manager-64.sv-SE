...
**Tips**: På webbplatsen Adobe Developer finns följande artikel som handlar om hur du skapar ett ASP.NET-program som anropar tjänsten Forms och återger formulär. Se [Skapa ASP.NET-program](https://www.adobe.com/devnet/livecycle/articles/asp_net.html)för formuläråtergivning.
...
I följande kodexempel återges formuläret *Loan.xdp *på klienten med hjälp av Java API:t för Forms-tjänsten. Observera att formulärdesignen är en del av ett program och att den refereras genom att använda innehållsrots-URI-värdet `repository:///`. (Se [Återge Forms på klienten](/help/forms/developing/rendering-forms.md#rendering-forms-at-the-client).)
...
I följande kodexempel återges ett HTML-formulär med Forms Java API. Ett verktygsfält läggs till i HTML-formuläret samt två bifogade filer. Dessutom hämtas användaragentvärdet från `HttpServletRequest` objektet. (Se [Återge Forms som HTML](/help/forms/developing/rendering-forms.md#rendering-forms-as-html).)
...
* `renderPOForm`: Använder Forms Java API för tjänster för att återge ett dynamiskt inköpsorderformulär. Objektet `com.adobe.idp.Document` som returnerades av `convertDataSource` metoden används för att fylla i formuläret i förväg.

   Alla dessa metoder anropas inifrån Java-serverletens `doPost` metod. (Se [Fylla i Forms i förväg med flödeslayouter](/help/forms/developing/rendering-forms.md#prepopulating-forms-with-flowable-layouts).)
