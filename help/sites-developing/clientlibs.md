...
Som standard `cq:ClientLibraryFolder` kan placeras var som helst i `/apps`, `/libs` och `/etc` underträd i databasen (dessa standardinställningar och andra inställningar kan styras via **Bibliotekshanteraren Adobe Granite HTML** panelen [Systemkonsol](http://localhost:4502/system/console/configMgr)).
...
>[!NOTE]
>
>Statiska resurser under klientbiblioteksmappen måste finnas i en mapp som kallas *resurser*. Om du inte har statiska resurser, till exempel bilder, under mappen *resurser* kan det inte refereras till i en publiceringsinstans. Här är ett exempel: http://localhost:4503/etc.clientlibs/geometrixx/components/clientlibs/resources/example.gif
