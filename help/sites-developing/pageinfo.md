...
PageInfo-servleten returnerar information om resurser i databasen. Servern är bunden till URL:en `https://<server>:<port>/libs/wcm/core/content/pageinfo.json` och använder `path` parameter som identifierar resursen. Följande exempel-URL returnerar information om `/content/we-retail/us/en` nod:
