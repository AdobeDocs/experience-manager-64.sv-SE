...
För sökningar finns en integreringsfunktion som gör att du kan använda AEM sökning, sökning i e-handelssystemet, en sökning från tredje part eller en kombination av dessa.
...
Startpunkten för sökgränssnittet är `CommerceService#search` metod som returnerar en `CommerceResult` -objekt. Se [API-dokumentation](/help/sites-developing/ecommerce.md#api-documentation) om du vill ha mer information om det här avsnittet.
