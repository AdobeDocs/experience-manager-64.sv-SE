...
description: Lär dig integrera AEM med Dynamic Media Classic.
...
Experience Manager-användare kan välja mellan två lösningar för Dynamic Media:

* Använda din integrerade instans av Experience Manager med *Dynamic Media Classic*.
* Använda *Dynamic Media* som är integrerat i Experience Manager.

Använd följande kriterier för att avgöra vilken lösning som passar dig bäst:

* Om du är en **befintlig** Dynamic Media Classic-kunder vars mediefiler finns i Dynamic Media Classic för publicering och leverans, men du vill integrera dessa mediefiler med Sites (WCM)-redigering och/eller Experience Manager Assets för hantering, använder sedan [Experience Manager/Dynamic Media Classic-integration från punkt till punkt](#aem-scene-point-to-point-integration) som beskrivs i det här dokumentet.
...
Experience Manager Dynamic Media förenar Dynamic Media Classic funktioner direkt i Experience Manager Platform.
...
* **[!UICONTROL Interactive Media]** - **[!UICONTROL Interactive Media]** är för de karusellbanderoller, interaktiva bilder och interaktiva videor som har interaktivitet i dem, till exempel hotspot-områden eller bildscheman. Den här komponenten är smart. Beroende på om du lägger till en bild eller en video har du olika alternativ. Dessutom är visningsprogrammet responsivt. Skärmstorleken ändras alltså automatiskt baserat på skärmstorleken. Alla visningsprogram är HTML5-visningsprogram.
...
1. Följ de specifika instruktionerna för ditt användningsfall:
   ...
   <table> 
    ...
    <td><p>Ladda upp material till Experience Manager och använd Dynamic Media för att generera renderingar för nedladdning/delning. Publicera automatiskt Experience Manager Assets till Dynamic Media Classic för leverans.</p> <p><strong>Viktigt:</strong> Inaktiverar dubblettbearbetning och återgivningar som genereras i Experience Manager synkroniseras inte med Dynamic Media Classic</p> </td> 
    <td><p>På</p> <p>(Se steg 3)</p> </td> 
    <td>Av</td> 
    <td>Av</td> 
    <td><p><a href="#configuringautouploadingfromaemassets">På</a></p> <p>(Se steg 4)</p> </td> 
...
När du integrerar Dynamic Media Classic med Experience Manager finns det några viktiga tips att följa inom följande områden:
...
### Överföra resurser från Experience Manager Assets jämfört med Dynamic Media Classic {#uploading-assets-from-aem-assets-versus-from-scene}
...
Om resurserna redan finns i Dynamic Media Classic och de hanteras i ett gemensamt arkiv är det inte viktigt att du exporterar dem till Experience Manager Assets bara för att synkronisera dem tillbaka till Dynamic Media Classic för leverans, vilket är en onödig växling. I annat fall är det att behålla resurser i en enda databas och synkronisera till Dynamic Media Classic för leverans.
...
Om du vill konfigurera Dynamic Media Classic så att det integreras med Experience Manager måste du utföra följande steg:
...
1. Om du vill att Experience Manager-resurser ska överföras automatiskt till Dynamic Media Classic aktiverar du alternativet och lägger till Dynamic Media Classic i arbetsflödet för DAM-uppdateringsresurser. Du kan också överföra resurser manuellt.
1. Lägga till Dynamic Media Classic-komponenter i sidosparken. På så sätt kan användare använda Dynamic Media Classic-komponenter på sina Experience Manager-sidor.
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>Ad hoc-mapp</td> 
   <td>Resurser som ligger utanför målmappen och som överförs manuellt till Dynamic Media Classic i WCM-filen placeras automatiskt i en separat ad hoc-mapp på Dynamic Media Classic. Du konfigurerar den här funktionen i molnkonfigurationen i Experience Manager.</td> 
  </tr> 
  <tr> 
   <td>Blandat media</td> 
  ...
  <tr> 
   ...
   <td><p>Om du använder undermappar i målmappen ska du antingen använda unika namn för varje resurs (oavsett plats) eller konfigurera Dynamic Media Classic (under Konfigurera) så att resurser inte skrivs över oavsett plats.</p> <p>Annars överförs resurser med samma namn som överförs till en Dynamic Media Classic-målundermapp, men resursen med samma namn i målmappen tas bort. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
Om du navigerar till `/etc/cloudservices/scene7/endpoints.html`visas servrarna i listan (och du kan redigera dem genom att klicka på URL:en):
...
>
>Om du vill testa kör integreringen ska rotmappen bara peka på en undermapp, i stället för på hela företaget.

>[!NOTE]
>
>Du kan ha flera konfigurationer: en molnkonfiguration representerar en användare på ett Dynamic Media Classic-företag. Om du vill få tillgång till andra Dynamic Media Classic-företag eller -användare skapar du flera konfigurationer.

Så här konfigurerar du Experience Manager så att det kan publicera resurser till Dynamic Media Classic:

1. Tryck på ikonen Experience Manager och navigera till **[!UICONTROL Deployment > Cloud Services]** för att få tillgång till Adobe Dynamic Media Classic.

1. Tryck på **[!UICONTROL Configure now]**.

   ![chlimage_1-297](assets/chlimage_1-297.png)

1. I **[!UICONTROL Title]** -fält och eventuellt i **[!UICONTROL Name]** anger du lämplig information. Tryck på **[!UICONTROL Create]**.
...
1. Ange e-postadress, lösenord och region för ditt Dynamic Media Classic-konto och tryck **[!UICONTROL Connect to Dynamic Media Classic]**. Du är ansluten till Dynamic Media Classic-servern och dialogrutan utökas med fler alternativ.
...
1. Tryck på [!UICONTROL Tools] ikonen, navigera sedan till **[!UICONTROL Operations > Web Console]**. Webbkonsolen öppnas.
1. Navigera till **[!UICONTROL Adobe CQ Dynamic Media Classic Dam Listener]** och väljer **[!UICONTROL Enabled]** kryssruta.
...
När en Experience Manager-instans har konfigurerats för att hantera videokodning via Dynamic Media Classic finns som standard en timeout på 35 minuter för alla överföringsjobb. Om du vill att videokodningsjobb som kan ta längre tid ska kunna köras ska du konfigurera den här inställningen:
...
1. Ändra talet som du vill i dialogrutan **[!UICONTROL Active job timeout]** fält. Ett icke-negativt tal accepteras med måttenheten i sekunder. Som standard är det här talet 2100.
   ...
   >[!NOTE]
   >
   >Bästa praxis: De flesta resurser är kapslade inom några minuter (till exempel bilder). Men i vissa fall, till exempel större videor, ökar du timeoutvärdet till 7 200 sekunder (två timmar) för att få plats med lång bearbetningstid. Annars markeras det här Dynamic Media Classic-överföringsjobbet som **[!UICONTROL UploadFailed]** i JCR-metadata.

1. Tryck på **[!UICONTROL Save]**.

### Automatisk uppladdning från Experience Manager Assets {#autouploading-from-aem-assets}

Från och med Experience Manager 6.3.2 är Experience Manager Assets nu konfigurerat för dig så att alla digitala resurser som du överför till den digitala resurshanteraren automatiskt uppdateras till Dynamic Media Classic om resurserna finns i en CQ-målmapp.
...
Så här konfigurerar du automatisk uppladdning från Experience Manager Assets:

1. Tryck på ikonen Experience Manager och navigera till **[!UICONTROL Deployment > Cloud Services]** under rubriken Dynamic Media, under Available Configurations, tryck **[!UICONTROL dms7 (Dynamic Media]**)
1. Tryck på **[!UICONTROL Advanced]** väljer du **[!UICONTROL Enable Automatic Upload]** kryssrutan och tryck sedan på **[!UICONTROL OK]**. Du måste nu konfigurera arbetsflödet för DAM-resurser så att det omfattar överföring till Dynamic Media Classic.
...
1. Gå tillbaka till välkomstsidan för Experience Manager och tryck på **[!UICONTROL Workflows]**. Dubbelklicka på **DAM-uppdateringsresurs** arbetsflöde för att öppna det.
1. Navigera till **[!UICONTROL Workflow]** komponenter, och markera **[!UICONTROL Dynamic Media Classic]**. Dra **[!UICONTROL Dynamic Media Classic]** till arbetsflödet och tryck **[!UICONTROL Save]**. Resurser som läggs till i Experience Manager Assets i målmappen överförs automatiskt till Dynamic Media Classic.
...
Om du vill publicera mediefiler direkt när du överför dem till Dynamic Media Classic behöver du inte konfigurera några alternativ. Detta är standardbeteendet.
...
1. [Använd Admin Console för att skapa ett supportärende.](https://helpx.adobe.com/enterprise/admin-guide.html/enterprise/using/support-for-experience-cloud.ug.html) I ditt supportärende begär du att en säker förhandsgranskning är aktiverad för ditt Dynamic Media Classic-konto.
1. Följ anvisningarna för att [Konfigurera säker förhandsgranskning för ditt Dynamic Media Classic-konto.](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish)
...
1. Tryck på ikonen Experience Manager och navigera till **[!UICONTROL Deployment > Cloud Services]**, trycka **[!UICONTROL Dynamic Media Classic]** och väljer din konfiguration i Dynamic Media Classic.
1. Tryck på fliken **[!UICONTROL Advanced]**. I **[!UICONTROL Enable Secure View]** nedrullningsbar meny, välja **[!UICONTROL Upon AEM Publish Activation]** för att skicka material till Dynamic Media Classic utan publicering. (Som standard är det här värdet inställt på **[!UICONTROL Immediately]**, där Dynamic Media Classic resurser publiceras omedelbart.)

   Se [Dynamic Media Classic-dokumentation](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish) om du vill ha mer information om hur du testar resurser innan du publicerar dem.
...
Navigera till en Dynamic Media Classic-komponent på en sida i Experience Manager för att se om säker förhandsvisning är aktiverat. Tryck på **[!UICONTROL Edit]**. Resursen har den säkra förhandsvisningsservern som anges i URL:en. När du har publicerat i Experience Manager uppdateras serverdomänen i filreferensen från förhandsgransknings-URL:en till produktions-URL:en.
...
1. Tryck på ikonen Experience Manager och navigera till **[!UICONTROL Sites]** och sedan rotsidan för din webbplats (inte språkspecifik).

1. I verktygsfältet väljer du [!UICONTROL settings] ikon och tryck **[!UICONTROL Open Properties]**.

1. Tryck **[!UICONTROL Cloud Services]** och trycka **[!UICONTROL Add Configuration]** och markera **[!UICONTROL Dynamic Media Classic]**.
1. I **[!UICONTROL Adobe Dynamic Media Classic]** väljer du önskad konfiguration och trycker på **[!UICONTROL OK]**.
...
1. I Experience Manager: tryck **[!UICONTROL Websites]** och navigera till webbplatsens rotsida (inte språkspecifik).

1. Tryck på **[!UICONTROL Page]** ikon och tryck **[!UICONTROL Page Properties]**.
...
1. Tryck på ikonen Experience Manager och navigera till **[!UICONTROL Deployment > Cloud Services]**, trycka **[!UICONTROL Dynamic Media Classic]** och väljer din konfiguration i Dynamic Media Classic.
1. Om du vill öppna konfigurationen trycker du på **[!UICONTROL Edit]**.
...
1. Tryck på fliken **[!UICONTROL Advanced]**. I **[!UICONTROL Ad-hoc Folder]** -fält kan du ändra **Ad hoc** mapp. Som standard är det **namn_på_företaget/CQ5_adhoc**.
...
1. På panelen Konfiguration av Adobe Experience Manager Web Console på **[!UICONTROL OSGi]** meny, tryck **[!UICONTROL Configuration]**.
1. Sök efter och tryck i kolumnen Namn **[!UICONTROL Adobe CQ Dynamic Media Classic Asset MIME type Service]** för att redigera konfigurationen.
...
1. Till höger på CRXDE Lite-sidan trycker du på **[!UICONTROL Properties]** -fliken.
1. Ange en Dynamic Media Classic-parameter för överföringsjobb i **[!UICONTROL jobParam]** värdefält.
   ...
   >
   >Kontrollera att PSD-filen innehåller&quot;lager&quot;. Om det bara är en bild eller en bild med mask, bearbetas den bara som en bild eftersom det inte finns några lager att bearbeta.

1. Tryck på i det övre vänstra hörnet av CRXDE Lite-sidan **[!UICONTROL Save All]**.

## Felsökning av integrering mellan Dynamic Media Classic och Experience Manager {#troubleshooting-scene-and-aem-integration}
...
* Kontrollera att resursen du försöker överföra finns i **[!UICONTROL CQ target]** (du anger den här mappen i Dynamic Media Classic molnkonfiguration).
* Om så inte är fallet måste du konfigurera molnkonfigurationen i **[!UICONTROL Page Properties]** för den sidan, tillåta överföring till **[!UICONTROL CQ adhoc]** mapp.
