...
Efter den [lokala uppgraderingen](/help/sites-deploying/in-place-upgrade.md) ska följande aktiviteter utföras för att slutföra uppgraderingen. Det antas att AEM har startats med 6.4-behållaren och att den uppgraderade kodbasen har distribuerats.
...
* [Granska mappen PreUpgradeBackup](#inspect-preupgradebackup-folder)
...
Använd alla relevanta AEM 6.4-servicepaket om de har släppts.

### Migrera AEM-funktioner {#migrate-aem-features}

Flera funktioner i AEM kräver ytterligare steg efter uppgraderingen. En fullständig lista över de här funktionerna och stegen för att migrera dem i AEM 6.4 finns på sidan [Uppgraderingskod och anpassningar](/help/sites-deploying/upgrading-code-and-customizations.md) .
...
Det här avsnittet innehåller några problemscenarier som man kan ställas inför under uppgraderingsprocessen till AEM 6.4.
...
### Återskapa konfigurationen för Dynamic Media Cloud efter uppgradering {#dynamic-media-cloud-configuration}

Efter uppgradering till AEM 6.4 från en tidigare version kan Dynamic Media Cloud Configuration från tidigare inställningar bli otillgänglig från AEM 6.4 TouchUI. Du löser det här problemet genom att använda CRXDE Lite för att ta bort de tidigare inställningarna och sedan skapa en ny Dynamic Media Cloud-konfiguration. Se även [Dynamic Media-databasomstrukturering i AEM 6.4](/help/sites-deploying/dynamicmedia-repository-restructuring-in-aem-6-4.md).
...
Vi rekommenderar också att du jämför paketlistan för en ny AEM 6.4-instans med den uppgraderade instansen för att identifiera de paket som inte uppgraderats. Detta ger en närmare beskrivning av vad du ska söka efter i `error.log`.
