...
* `db`: Namn på Mongo-databasen. Standardvärdet är **Oak** . Nya AEM 6-installationer använder dock **aem-author** som standarddatabasnamn.
...
* `path`: Sökväg till den katalog som filerna ska lagras i. Om det anges har det företräde framför `repository.home` värde.
...
>[!NOTE]
>
>När du använder AEM 6.4 med tarMK lagras binärfiler som standard i `FileDataStore`. Om du vill använda tarMK med S3-datastore måste du börja AEM med `crx3tar-nofds` runmode, till exempel:
...
1. Om AEM redan har konfigurerats för att fungera med lagringen tar du bort alla befintliga konfigurationsfiler från `aem-install/crx-quickstart/install` innan du fortsätter. De filer som behöver tas bort är:
...
>[!NOTE]
>
>När du använder AEM 6.4 med tarMK lagras binärfiler som standard i FileDataStore. Om du vill använda tarMK med Azure DataStore måste du börja AEM med `crx3tar-nofds` runmode, till exempel:
