...
Dokumentnodarkivet är grunden för AEM MongoMK-implementering. Det använder `org.apache.jackrabbit.oak.plugins.document.DocumentNodeStoreService` **PID**. Följande konfigurationsalternativ är tillgängliga:
...
* `db`: Namn på Mongo-databasen. Standardvärdet är **Oak**. Vid nya AEM 6-installationer används **aem-author** som standarddatabasnamn.
...
För att aktivera S3-datalagrets funktioner måste ett funktionspaket som innehåller S3 Datastore Connector hämtas och installeras. Gå till [Adobe-databasen](https://repo.adobe.com/nexus/content/groups/public/com/adobe/granite/com.adobe.granite.oak.s3connector/) och hämta den senaste versionen från 1.8.x-versionerna av funktionspaketet (till exempel com.adobe.granite.oak.s3connector-1.8.0.zip). Dessutom måste du hämta och installera det senaste AEM Service Pack som finns på sidan [AEM 6.4 Service Pack Release Notes](https://helpx.adobe.com/experience-manager/6-4/release-notes/feature-packs-release-notes.html).

>[!NOTE]
>
>När du använder AEM 6.4 med tarMK lagras binärfiler som standard i `FileDataStore`. Om du vill använda tarMK med S3-datastore måste du starta AEM med runmode `crx3tar-nofds`, till exempel:
...
>[!NOTE]
>
>När du använder AEM 6.4 med tarMK lagras binärfiler som standard i FileDataStore. Om du vill använda tarMK med Azure DataStore måste du starta AEM med runmode `crx3tar-nofds`, till exempel:
