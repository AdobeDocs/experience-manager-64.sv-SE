...
* **Installationsmapparnas namn regexp** och **maximalt hierarkidjup för installationsmappar** - ange var och i vilket djup databasmappar ska sökas efter resurser som ska installeras. När ett jokertecken används (som i .&amp;ast;/install) alla matchningar söks igenom, till exempel `/libs/sling/install` och `/libs/cq/core/install`.
...
**Dag CQ WCM-felsökningsfilter** Detta är användbart vid utveckling eftersom det går att använda suffix som ?debug=layout vid åtkomst av en sida. http://localhost:4502/cf#/content/geometrixx/en/support.html?debug=layout tillhandahåller till exempel layoutinformation som kan vara av intresse för utvecklaren.
...
* **URL för att skicka data** för att konfigurera den URL som används för att spåra sidstatistik (är avgörande om en spårningsbegäran går igenom dispatchern). Standardvärdet är till exempel `http://localhost:4502/libs/wcm/stats/tracker`.
